...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
            "--task",
            type=str,
            default="inpaint_retina",
            choices=['realsr', 'bicsr', 'inpaint_imagenet', 'inpaint_face', 'faceir', 'inpaint_retina', 'retinasr'],
            help="Chopping forward.",
            )
...
def get_configs(args):
    ...
    elif args.task == 'inpaint_retina':
        configs = OmegaConf.load('./configs/inpaint_lama256_retina.yaml')
        assert args.scale == 1, 'Please set scale equals 1 for image inpainting!'
        ckpt_url = _LINK[args.task]
        ckpt_path = ckpt_dir / f'resshift_{args.task}_s{_STEP[args.task]}.pth'
        vqgan_url = _LINK['vqgan_face256']
    ...
    elif args.task == 'retinasr':
        configs = OmegaConf.load('./configs/realsr_swinunet_realesrgan256_retina.yaml')
        assert args.scale == 4, 'We only support the 4x super-resolution now!'
        ckpt_url = _LINK[args.task]
        ckpt_path = ckpt_dir / f'resshift_{args.task}x{args.scale}_s{_STEP[args.task]}.pth'
        vqgan_url = _LINK['vqgan']
...
# python -m torch.distributed.launch --nproc_per_node 1 inference_resshift_4dsr.py -i /home/data/4DRADAR/ResShift/data/testdata/retina/lq -o /home/data/4DRADAR/ResShift/results/retina5cm_mask_infer --mask_path /home/data/4DRADAR/ResShift/data/testdata/retina/mask --task inpaint_retina --scale 1
# python -m torch.distributed.launch --nproc_per_node 1 inference_resshift_4dsr.py -i /home/data/4DRADAR/ResShift/data/testdata/retina_infersr/lq -o /home/data/4DRADAR/ResShift/results/retinasr_infer --task retinasr --scale 4
