...
class InpaintingDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_lq = util_image.imread(im_path.replace('hq','lq'), chn='rgb', dtype='float32')
        im_lq = self.transform(im_lq)
        out_dict['lq'] = im_lq
        #out_dict['mask'] = (mask - self.mean) / self.std               # c x h x w, [-1,1]
        im_mask = util_image.imread(im_path.replace('hq','mask_{}'.format(self.masknum)), chn='gray', dtype='float32')
